...
// TestIPToUint32_IPv6 verifies that an IPv6-only address (not representable as
// IPv4) returns 0 rather than panicking.
func TestIPToUint32_IPv6(t *testing.T) {
	ip := net.ParseIP("::1")
	got := ipToUint32(ip)
	if got != 0 {
		t.Errorf("expected 0 for IPv6 address, got 0x%08X", got)
	}
}

// TestIPToUint32_Nil verifies that a nil net.IP returns 0.
func TestIPToUint32_Nil(t *testing.T) {
	got := ipToUint32(nil)
	if got != 0 {
		t.Errorf("expected 0 for nil IP, got 0x%08X", got)
	}
}
